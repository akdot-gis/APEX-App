def run_manager_app():
    ...
    def _current_project_label_from_session():
        ...
        in-session project state first. This lets Source-tab updates immediately
        change the header after Proj_Name is updated.
        """
    ...
    # Header
    # ─────────────────────────────────────────────────────────────
    title_markdown("MANAGE AN EXISTING APEX PROJECT")
    st.write('')
    ...
    if st.session_state.get("guid"):
        if st.session_state.get("apex_error"):
            ...
            options = ["SOURCE", "INFORMATION", "FOOTPRINT", "TRAFFIC IMPACTS", "COMMUNITIES", "DEPLOYMENT"]
            ...
            # Define tab callables. Only the chosen one will run.t
            def _tab_source():
